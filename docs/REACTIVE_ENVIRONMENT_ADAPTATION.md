...
参考变化率限制避免了“τ=0 直接跳到 τ*”式切换。自动检测默认预热 0.5 s；冲击需要至少两个接触且满足 IMU 加速度阈值；打滑需要持续速度误差。冲击信号有释放滞回，避免同一次冲击在高频重复触发。

事件脚本时间以 gait 开始为零，不是进程启动时间。`--reactive-events` 可在没有脚本时单独开启传感器检测；`--event-script path` 用于确定性复现实验。
