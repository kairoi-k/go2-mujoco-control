...
- 2026-08-21 自动高度图验收（最终二进制复核）：无障碍基线与真实碰撞障碍两组严格通过；地图有效率 100%、最大年龄 20 ms；障碍检测延迟 40 ms，自动选择 obstacle_left，机身横移 0.445 m，障碍物接触次数/法向力均为 0。
- 2026-08-21 物理冲击自动验收（最终二进制复核）：0.8 m/s 仿真冲击在 state_tick=8.002 s 被识别，2 ms 后进入 impact，0.5 s 后进入 emergency_stop；最大速度突变 0.803 m/s，最大 roll 0.0868 rad、pitch 0.1738 rad，WBC 残差 1.6964e-5，全部状态码为 0。
