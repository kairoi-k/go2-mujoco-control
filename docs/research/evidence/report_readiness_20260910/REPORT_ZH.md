...
从历史归档恢复全部15次变速原始CSV，核对它们均记录为干净源码07cbc7efdd17b9960897e277f401654c46410e48。五份指令配置与该Git版本逐字节一致；但原始运行未记录profile哈希，不能补造当时绑定。归档SHA-256与保留清单相符。用独立稳定保持检查重审，并另写不调用该分析器的逐窗口数值检查，42个过渡判断全部一致。分析器10个反例/边界测试通过。
...
四脚都曾在台面形成持续支撑，身体与四脚完成离台，没有非足部台阶碰撞或安全停机；交互阶段速度中位数约0.850米/秒。今晚重放平地、越障及归因调试数据，汇总结果与已提交历史results.json完全一致，原始输入哈希也一致；调试组仅用于归因，不算重复试验。

但四腿均出现非台面接触，7个完整交互周期仅1个满足保留的跑步拓扑，状态与指令时钟漂移28.318毫秒，超过该版本20毫秒门槛。前脚首次接触前的关键窗口没有可用地形执行计划，因此不能把成功归功于主动地形规划。可以说“完成过5厘米跨越”，不能说“可靠的地形感知动态越障已解决”。同源码平地周期诊断为46/49合格，这不是完整B1验收，但越障尚无今晚新增复跑或成功率统计。
...
这一实验暴露了短窗口可行仍可能无法续接的问题；它不是对物理无解的证明。当前主要问题是续接可行性、运动选择自由度和实时计算，而不是继续把旧分析器调绿。求解器主路线尚待论文与成熟项目对比选型，不能宣称已选定最佳方案。
