...
if lp.success:
 witness=center+T@lp.x;report['independent_feasible_witness']=residual(witness)
 sol=minimize(lambda y:.5*y@y,lp.x,jac=lambda y:y,constraints=LinearConstraint(C,-np.inf,rhs),method='SLSQP',options={'ftol':1e-10,'maxiter':2000})
 x=center+T@sol.x;report.update(qp_success=bool(sol.success),qp_message=sol.message,qp_iterations=int(sol.nit),qp_residual=residual(x),solution=x.tolist())
pathlib.Path(a.out).write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({k:v for k,v in report.items() if k!='solution'},indent=2))
