...
def main():
 ...
 import constrained_whole_body_feedback as constrained
 p=argparse.ArgumentParser();p.add_argument('trajectory');p.add_argument('--out',required=True);p.add_argument('--periods',type=int,default=5);p.add_argument('--initial-vy',type=float,default=0);p.add_argument('--initial-roll',type=float,default=0);p.add_argument('--r-weight',type=float,default=.01);p.add_argument('--constrained',action='store_true');a=p.parse_args()
 ...
 with open('/tmp/go2_mujoco_experiment.lock','a') as lock:
  ...
  constraint_checks=[];control_failure=None
  for step in range(N*a.periods):
   k=step%N;cycle=step//N;qref=nom[k].qpos.copy();qref[0]+=cycle*r['command_vx']*r['period_s']
   begin=time.perf_counter_ns();error=state_error(m,qref,nom[k].qvel,d.qpos,d.qvel);requested=u[k]-Ks[k]@error
   if a.constrained:
    try:
     cmd,check=constrained.constrained_tracking(m,d,requested,gids)
     check['step']=step;check['time']=float(d.time);constraint_checks.append(check)
    except constrained.ConstrainedFeedbackFailure as exc:
     control_failure={'step':step,'time':float(d.time),'diagnostic':exc.diagnostic}
     break
   else:cmd=np.clip(requested,-35,35)
   latencies.append((time.perf_counter_ns()-begin)/1000);saturation+=int(np.any(abs(requested)>35));tracking.append(float(np.linalg.norm(error)))
   d.ctrl[:]=cmd;mujoco.mj_step(m,d);obs=copy.copy(d);mujoco.mj_forward(m,obs);controls.append(cmd.tolist())
   rows.append({'time':float(obs.time),'qpos':obs.qpos.tolist(),'qvel':obs.qvel.tolist(),'normal_forces':forces(m,obs,gids).tolist(),'torque':obs.qfrc_actuator[va].tolist()})
  out={k:v for k,v in r.items() if k not in ('rows','controls','solver','terminal_reference','initial_integration_state','source_sha','curation')};out.update({'source_sha':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'scope':'periodic whole-body state-feedback initialized replay; privileged model, not live controller or B1','initial_integration_state':saved_initial.tolist(),'terminal_reference':terminal,'controls':controls,'rows':rows,'feedback':{'method':('one-step hard-force-constrained full-model torque tracking of TVLQR' if a.constrained else 'periodic time-varying discrete LQR with applied35Nm clipping'),'constrained':a.constrained,'constraint_checks':constraint_checks,'control_failure':control_failure,'completed_steps':len(rows),'nominal_certificate':nominal_certificate,'transition_derivative_checks':derivative_checks,'nominal_terminal_reference':nominal_terminal,'nominal_terminal_tangent_error':state_error(m,np.array(nominal_terminal['qpos']),np.array(nominal_terminal['qvel']),d.qpos,d.qvel).tolist(),'v1_terminal_reference_scope':'perturbed initial translated as frozen V1 requires; nominal recovery is separately reported','r_weight':a.r_weight,'riccati_iterations':iterations,'riccati_relative_residual':riccati_residual,'nominal_wrap_tangent_error':wrap_error.tolist(),'periods':a.periods,'periodic_orbit_certified':False,'spectral_radius_scope':'repeated nominal local linear maps; nonzero wrap defect prevents a periodic-orbit stability claim','linear_period_map_spectral_radius':radius,'setup_s':setup,'feedback_latency_us':dict(zip(['p50','p95','max'],[float(np.percentile(latencies,50)) if latencies else None,float(np.percentile(latencies,95)) if latencies else None,max(latencies,default=None)])),'saturated_samples':saturation,'max_state_tangent_error_norm':max(tracking,default=0.),'initial_vy_perturbation_mps':a.initial_vy,'initial_roll_perturbation_rad':a.initial_roll,'gains':[K.tolist() for K in Ks]}})
  ...
  if a.constrained:
   constrained_path=pathlib.Path(constrained.__file__).resolve()
   out['input_hashes'][str(constrained_path)]=hashlib.sha256(constrained_path.read_bytes()).hexdigest()
  ...
  print(json.dumps({'out':a.out,'feedback':{k:v for k,v in out['feedback'].items() if k!='gains'},'peak_force':max((max(r['normal_forces']) for r in rows),default=0.),'end_omega':d.qvel[3:6].tolist()},indent=2))
