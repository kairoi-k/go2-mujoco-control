...
int main(int argc,char**argv){
 ...
 try {
  ...
  int count;input>>count;std::vector<double> state(count);for(double& value:state)input>>value;
  if(count!=mj_stateSize(m.get(),mjSTATE_INTEGRATION))throw std::runtime_error("state size mismatch");
