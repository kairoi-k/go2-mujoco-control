...
import contextlib,io,runpy,pathlib,json,re,hashlib
import numpy as np
...
out={'scope':'conditional instantaneous target oracle; impact and full trajectory unverified','lp_status':int(lp.status),'lp_message':lp.message,'normal_terminal_velocity_bounds_mps':[-2,0],'script_sha256':hashlib.sha256(pathlib.Path(__file__).read_bytes()).hexdigest()}
if lp.success:
