// Early single-leg joint-space lift demo (pre real_leg_lift).
#include <array>
#include <atomic>
#include <chrono>
#include <cmath>
...
void TrackingExperiment::LowCmdWrite()
{
    ...
    {
        double motion_time = running_time_ - kStandUpDuration;
        double lift = 0.20 * 0.5 * (1.0 - std::cos(2.0 * M_PI * 0.5 * motion_time));

        for (int i = 0; i < kMotorCount; i++)
        {
            double q_target = stand_up_joint_pos_[i];
            ...
            low_cmd_.motor_cmd()[i].q() = q_target;
            low_cmd_.motor_cmd()[i].dq() = 0;
