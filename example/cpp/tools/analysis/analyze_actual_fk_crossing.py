...
def gt_time_quality(times, invalid, input_times):
    sorted_deltas = [right-left for left, right in zip(times, times[1:])]
    input_deltas = [right-left for left, right in zip(input_times, input_times[1:])]
    return {"rows_total": len(times)+invalid, "valid_time_rows": len(times), "invalid": invalid,
            "input_nonmonotonic": any(delta < 0 for delta in input_deltas),
            "sorted_non_decreasing": all(delta >= 0 for delta in sorted_deltas),
            "duplicates": sum(delta == 0 for delta in sorted_deltas),
            "sort_key": "(time_s,row_index)"}
...
def analyze(data_path, gt_path=None):
    ...
    gt_rows, gt_times, gt_invalid, gt_input_times = [], [], 0, []
    if gt_path:
        _, raw_gt = load_rows(gt_path)
        pairs = [(number(row, "time_s"), index, row) for index, row in enumerate(raw_gt)]
        gt_input_times = [time for time, _, _ in pairs if time is not None]
    ...
    matched_gt_indices = {nearest_index(gt_times, number(row, "state_tick_s")) for row in valid_rows
                          if gt_rows and nearest_index(gt_times, number(row, "state_tick_s")) is not None
                          and abs(gt_times[nearest_index(gt_times, number(row, "state_tick_s"))]-number(row, "state_tick_s")) <= GT_ALIGNMENT_TOLERANCE_S}
    gt_alignment = {"status": "checked" if gt_aligned and max(all_deltas, default=0.0) <= GT_ALIGNMENT_TOLERANCE_S else "unavailable", "mode": "sorted (time_s,row_index) pairs; nearest state_tick_s to GT time_s; nearest mapping is audit-only", "rows_aligned": gt_aligned, "matched_state_rows": gt_aligned, "unmatched_state_rows": len(valid_rows)-gt_aligned, "matched_gt_rows": len(matched_gt_indices), "unmatched_gt_rows": len(gt_rows)-len(matched_gt_indices), "nearest_dt_median_s": statistics.median(all_deltas) if all_deltas else None, "nearest_dt_p95_s": percentile(all_deltas,.95), "nearest_dt_max_s": max(all_deltas, default=None), "tolerance_s": GT_ALIGNMENT_TOLERANCE_S, "reason": None if gt_aligned and max(all_deltas, default=0.0) <= GT_ALIGNMENT_TOLERANCE_S else "missing/over-tolerance GT alignment"}
