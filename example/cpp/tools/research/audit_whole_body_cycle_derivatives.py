...
def local_axis_audit(pb, x, nominal, S):
    k = 32
    before = copy.copy(nominal[k-1])
    before.ctrl[:] = x.reshape(pb.blocks, 12)[k//pb.bs]
    center = copy.copy(before)
    mujoco.mj_step(pb.m, center)
    rng = np.random.default_rng(1301)
    v = rng.normal(size=x.size)
    v /= np.linalg.norm(v)
    du = v.reshape(pb.blocks, 12)[k//pb.bs]
    dx = S[k-1] @ v
    def perturb(z):
        d = copy.copy(before)
        mujoco.mj_integratePos(pb.m, d.qpos, z[:18], 1.)
        d.qvel[:] += z[18:36]
        d.ctrl[:] += z[36:]
        mujoco.mj_step(pb.m, d)
        return d
    def meta(d):
        return {'ncon': int(d.ncon), 'nefc': int(d.nefc),
                'solver_niter': np.asarray(d.solver_niter).tolist(),
                'contacts': [{'geom1': int(c.geom1), 'geom2': int(c.geom2), 'dist': float(c.dist)} for c in d.contact]}
    results = []
    for eps in (1e-4, 1e-5, 1e-6, 1e-7, 1e-8):
        A, B = np.empty((36,36)), np.empty((36,12))
        mujoco.mjd_transitionFD(pb.m, copy.copy(before), eps, True, A, B, None, None)
        native = np.hstack([A,B])
        manual = np.empty((36,48))
        columns = []
        for col in range(48):
            z = np.zeros(48)
            z[col] = eps
            plus, minus = perturb(z), perturb(-z)
            manual[:,col] = (tangent(pb.m, center, plus, 1.)-tangent(pb.m, center, minus, 1.))/(2*eps)
            columns.append({'column': col, 'native_vs_manual': stats(native[:,col], manual[:,col]),
                            'native_norm': float(np.linalg.norm(native[:,col])),
                            'manual_norm': float(np.linalg.norm(manual[:,col])),
                            'plus': meta(plus), 'minus': meta(minus)})
        direction = np.r_[dx,du]
        plus, minus = perturb(eps*direction), perturb(-eps*direction)
        direct = (tangent(pb.m, center, plus, 1.)-tangent(pb.m, center, minus, 1.))/(2*eps)
        results.append({'epsilon': eps, 'native_vs_manual_matrix': stats(native,manual),
            'native_vs_directional': stats(native@direction,direct),
            'manual_vs_directional': stats(manual@direction,direct),
            'directional_plus': meta(plus), 'directional_minus': meta(minus), 'columns': columns})
    return {'step': k, 'pre_time': float(before.time), 'post_time': float(center.time),
        'method': '48 independent centered axes over tangent q, v, u with original pre-step integration memory; compare native transitionFD and direct mj_step. Direction is chained S31 times seed1301 direction0 plus corresponding block control direction.',
        'nominal': meta(center), 'results': results}
...
def main():
    ...
    parser.add_argument('--checked-transitions', action='store_true')
    ...
    with open('/tmp/go2_mujoco_experiment.lock', 'a') as lock:
        ...
        pb = CycleProblem(args.source, args.seed, args.scene, force_target=args.force_target_n, transition_mode="checked" if args.checked_transitions else "native")
        ...
        observations, S = rollout(pb.m, pb.initial, x.reshape(pb.blocks, 12), pb.bs, True, transition_mode=pb.transition_mode)
        nominal = trace(pb, x)
        ...
        local_axes = local_axis_audit(pb, x, nominal, S)
        ...
        report = {'schema': 'whole-body-derivative-audit-v1', 'mujoco_version': mujoco.__version__,
            ...
            'transition_mode': pb.transition_mode, 'checked_transition_diagnostics': pb.derivative_diagnostics, 'local_axis_audit': local_axes, 'checks': checks, 'repeated_forward': repeated_forward, 'elapsed_s': time.perf_counter()-started}
