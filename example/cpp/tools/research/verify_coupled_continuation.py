...
def main():
 ...
 packet_path=next(pathlib.Path(f) for f in audit['input_hashes'] if pathlib.Path(f).name=='trajectory.packet')
 manifest=json.loads((packet_path.parent/'manifest.json').read_text());nominal=json.loads(pathlib.Path(manifest['source_hashes']['nominal']['path']).read_text())
 refs=[np.asarray(line.split()[3:],dtype=float) for line in packet_path.read_text().splitlines() if line.startswith('sample ')]
 if len(refs)!=70 or any(v.shape!=(481,) for v in refs):raise ValueError('reference coverage')
 reports={}
 with open('/tmp/go2_mujoco_experiment.lock','a') as lock:
  ...
  for name,case in audit['results'].items():
   ...
   residual=force_error=clock_error=law_error=0.;peak_force=peak_tau=peak_speed=peak_pose=peak_bad=0.;min_height=1e9;joint_violation=0.;negative=False
   rows=case['rows']
   if len(rows)!=156:raise ValueError('incomplete continuation')
   for k,row in enumerate(rows):
    tau=np.asarray(row['tau'])
    if k<16:expected=np.asarray(head['baseline_controls'] if name=='baseline' else head['solver']['controls'])[k]
    else:
     ref=refs[k%70];q=ref[:19].copy();q[0]+=(k//70)*nominal['command_vx']*nominal['period_s'];err=np.empty(36);mujoco.mj_differentiatePos(m,err[:18],1.,q,d.qpos);err[18:]=d.qvel-ref[19:37];expected=np.clip(ref[37:49]-ref[49:].reshape(12,36)@err,-35,35)
    law_error=max(law_error,float(max(abs(tau-expected))))
    pre=copy.copy(d);pre.ctrl[:]=tau;mujoco.mj_forward(m,pre);d.ctrl[:]=tau;mujoco.mj_step(m,d);post=copy.copy(d);mujoco.mj_forward(m,post)
   ...
   checks={'head_and_feedback_law':law_error<=1e-12,'state_replay':residual<=1e-9,'force_replay':force_error<=1e-8,'clock':clock_error<=1e-10,'force':peak_force<=180 and not negative,'torque':peak_tau<=35,'joint_speed':peak_speed<=30,'joint_range':joint_violation<=0,'height':min_height>=.28,'pose':peak_pose<=np.pi/12,'nonfoot':peak_bad<=1e-6}
   reports[name]={'checks':checks,'passed':bool(all(checks.values())),'law_residual':law_error,'state_residual':residual,'force_residual':force_error,'clock_residual':clock_error,'peak_force':peak_force,'peak_torque':peak_tau,'peak_abs_roll_pitch_rad':peak_pose,'min_height':min_height,'terminal_velocity':d.qvel[:6].tolist()}
