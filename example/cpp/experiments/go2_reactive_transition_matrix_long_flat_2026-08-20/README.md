...
The event script holds each token for 4.0 s. `emergency_stop` is a terminal controller latch, so its red label can continue through the stop-hold tail; this is intentional and recorded in the manifest/metrics.
