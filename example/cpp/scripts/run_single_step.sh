...
timeout_s="${1:-35}"
experiment_name="${2:-go2_single_step_${3:-FR}_2026-08-02}"
lift_leg="${3:-FR}"
body_shift_x="${4:-}"
body_shift_y="${5:-}"
foot_lift_height="${6:-0.040}"
swing_x="${7:-0.100}"
swing_y="${8:-0.000}"
...
printf '\n' | "${controller}" \
  ...
  1 "${lift_leg}" "${swing_x}" "${swing_y}" \
  "${body_advance_x}" "${body_advance_y}" \
  >"${experiment_dir}/controller.log" 2>&1

stop_simulator
python3 "${cpp_dir}/tools/analysis/analyze_single_step.py" \
  "${experiment_dir}/data.csv" \
  --leg "${lift_leg}"

echo "Single-step experiment completed: ${experiment_dir}"
